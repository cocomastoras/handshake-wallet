...
const TOKENS = [
    new Token(8453, '0x940181a94A35A4569E4529A3CDfB74e38FD98631', 18, "Aero", 'Aero'),
    ...
    new Token(8453, '0x9E53e88dCff56d3062510A745952DEC4cEFDff9E', 18, "DOG", 'DOG'),
    new Token(8453, '0x4ed4E862860beD51a9570b96d89aF5E1B0Efefed', 18, "DEGEN", 'Degen'),
    new Token(8453, '0x532f27101965dd16442E59d40670FaF5eBB142E4', 18, "BRETT", 'Brett'),
    new Token(8453, '0xE3086852A4B125803C815a158249ae468A3254Ca', 18, "mfer", 'mfercoin'),
    new Token(8453, '0xE1aBD004250AC8D1F199421d647e01d094FAa180', 18, "ROOST", 'Roost Coin'),
...
async function deployRouter() {
    await reset("https://base-mainnet.g.alchemy.com/v2/xkIcuOrk0eDv9TaZxMXtkwptU91055bi", 21061331)
    ...
    const dataFetcher = await DataFetcher.connect(owner).deploy()
    const Router = await ethers.getContractFactory('RouterAeroUniBase')
    const router = await Router.connect(owner).deploy(await dataFetcher.getAddress(), await owner.getAddress(), await  feeSink.getAddress(), 10000, 10000)
    const tokens = TOKENS.map(tk => {
        return new ethers.Contract(tk.address, TOKEN_ABI, owner)
    })
    return {router, owner, feeSink, user, tokens, dataFetcher};
}
...
async function sellNativeForTokenMalformedVersion(routerContract, token, amountIn, slippage, account){
    ...
    const signer = routerContract.connect(account)
    const txn = await signer.sellNativeForToken(token, quote[1], 5, quote[2], commands, {value: amountIn})
    const rsp = await txn.wait();
    return rsp['hash']
}
...
async function sellTokenForNativeMalformedVersion(tokenContract, routerContract, amountIn, slippage, account){
    ...
    signer = routerContract.connect(account)
    const txn = await signer.sellTokenForNative(await tokenContract.getAddress(), amountIn, quote[1], 5, quote[2], commands)
    const rsp = await txn.wait();
    return rsp['hash']
}
describe("Trading Aero", function () {
    it('should get optimal path buy', async function(){
            ...
            for (let i=0; i<5;i++) {
               await getOptimalPath(router, await tokens[i].getAddress(), 10n**14n, 0 , 10000)
            }
    ...
    it('should get optimal path sell', async function(){
           const {
            ...
            for (let i=0; i<5;i++) {
              await getOptimalPath(router, await tokens[i].getAddress(), 100000n * 10n**toBigInt(TOKENS[i].decimals), 1 , 10000)
            }
        }).timeout(10000000000000000000000000000000);
    it('executes swaps for each token in the list', async function(){
         ...
         for (let i =0; i< 26; i++) {
             await sellNativeForToken(router, await tokens[i].getAddress(), 10n ** 14n, 30000, user)
             let balance = await tokens[i].balanceOf(await user.getAddress())
             expect(balance).to.be.greaterThan(0n)
             await sellTokenForNative(tokens[i], router, balance, 30000, user)
             balance = await tokens[i].balanceOf(await user.getAddress())
             expect(balance).to.be.equal(0n)
         }
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached uniV3 on buy', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        const quote = await getOptimalPath(router, await tokens[2].getAddress(), 10n ** 14n, 0, 10000)
        expect(quote[0]).equal(1n);
        await expect(sellNativeForTokenWithoutQuote(router, await tokens[2].getAddress(), 5n ** 14n, 10000, owner, quote)).revertedWithCustomError(router, 'V3TooLittleReceived()')
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached uniV2 on buy', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        const quote = await getOptimalPath(router, await tokens[3].getAddress(), 10n**14n, 0, 10000)
        expect(quote[0]).equal(0n);
        await expect(sellNativeForTokenWithoutQuote(router, await tokens[3].getAddress(),5n**14n, 10000, owner, quote)).revertedWithCustomError(router, "V2TooLittleReceived()")
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached aeroCL on buy', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        let quote = await getOptimalPath(router, await tokens[25].getAddress(), 10n ** 14n, 0, 10000)
        quote = [4n, quote[1], 200n, quote[3]]
        expect(quote[0]).equal(4n);
        await expect(sellNativeForTokenWithoutQuote(router, await tokens[25].getAddress(), 5n ** 14n, 10000, owner, quote)).revertedWithCustomError(router, 'V3TooLittleReceived()')
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached aeroStable on buy', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        const quote = await getOptimalPath(router, await tokens[0].getAddress(), 10n**14n, 0, 10000)
        expect(quote[0]).equal(3n);
        await expect(sellNativeForTokenWithoutQuote(router, await tokens[0].getAddress(),5n**14n, 10000, owner, quote)).revertedWithCustomError(router, "InsufficientOutputAmount()")
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached aeroVolatile on buy', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        const quote = await getOptimalPath(router, await tokens[6].getAddress(), 10n ** 14n, 0, 10000)
        expect(quote[0]).equal(2n);
        await expect(sellNativeForTokenWithoutQuote(router, await tokens[6].getAddress(), 5n ** 14n, 10000, owner, quote)).revertedWithCustomError(router, 'InsufficientOutputAmount()')
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached uniV3 on sell', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        await sellNativeForToken(router, await tokens[0].getAddress(), 10n**14n, 10000, owner)
        const balance = await tokens[0].balanceOf(await owner.getAddress())
        const quote = await getOptimalPath(router, await tokens[0].getAddress(), balance, 1, 10000)
        expect(quote[0]).equal(1n);
        await expect(sellTokenForNativeWithoutQuote(tokens[0], router, balance/2n, 10000, owner, quote)).revertedWithCustomError(router, 'V3TooLittleReceived()')
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached uniV2 on sell', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        await sellNativeForToken(router, await tokens[5].getAddress(), 10n**14n, 10000, owner)
        const balance = await tokens[5].balanceOf(await owner.getAddress())
        const quote = await getOptimalPath(router, await tokens[5].getAddress(), balance, 1, 10000)
        expect(quote[0]).equal(0n);
        await expect(sellTokenForNativeWithoutQuote(tokens[5], router, balance/2n, 10000, owner, quote)).revertedWithCustomError(router, 'V2TooLittleReceived()')
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached aeroCL on sell', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        await sellNativeForToken(router, await tokens[25].getAddress(), 10n ** 14n, 10000, owner)
        const balance = await tokens[25].balanceOf(await owner.getAddress())
        let quote = await getOptimalPath(router, await tokens[25].getAddress(), balance, 1, 10000)
        quote = [4n, quote[1], 200n, quote[3]]
        expect(quote[0]).equal(4n);
        await expect(sellTokenForNativeWithoutQuote(tokens[25], router, balance/2n, 10000, owner, quote)).revertedWithCustomError(router, 'V3TooLittleReceived()')
        }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached aeroStable on sell', async function(){
        const {
            router,
            owner,
            feeSink,
        ...
        quote = [3n, quote[1], 0n, quote[3]]
        expect(quote[0]).equal(3n);
        await expect(sellTokenForNativeWithoutQuote(tokens[0], router, balance/2n, 10000, owner, quote)).revertedWithCustomError(router, 'InsufficientOutputAmount()')
    }).timeout(10000000000000000000000000000000);
    it('should revert if slippage reached aeroVolatile on sell', async function(){
       const {
            router,
            owner,
            feeSink,
        ...
        await sellNativeForToken(router, await tokens[6].getAddress(), 10n**14n, 10000, owner)
        const balance = await tokens[6].balanceOf(await owner.getAddress())
        let quote = await getOptimalPath(router, await tokens[6].getAddress(), balance, 1, 10000)
        quote = [2n, quote[1], 0n, quote[3]]
        expect(quote[0]).equal(2n);
        await expect(sellTokenForNativeWithoutQuote(tokens[6], router, balance/2n, 10000, owner, quote)).revertedWithCustomError(router, 'InsufficientOutputAmount()')
        }).timeout(10000000000000000000000000000000);
    ...
    it('sends contract balance to fee sink', async function(){
        ...
        for (let i=0; i<10;i++) {
            const buyTxn = await sellNativeForToken(router, await tokens[i].getAddress(), 10n**14n, 100000, user)
    ...
    it('should have 0 trading fees if token is whitelisted', async function(){
        ...
        await expect(sellNativeForTokenReturnTxn(router, await tokens[0].getAddress(), 10n**14n, 100000, user)).to.emit(
            router, "SwapExecuted"
        ).withArgs(
            await user.getAddress(), WETH.address, await tokens[0].getAddress(), 10n**14n, 177602551139595874n, 0, 3, 0
        )
    }).timeout(10000000000000000000000000000000);
    ...
    it('should have trading fees if token is not whitelisted', async function(){
        ...
        await expect(sellNativeForTokenReturnTxn(router, await tokens[0].getAddress(), 10n**14n, 100000, user)).to.emit(
            ...
            await user.getAddress(), WETH.address, await tokens[0].getAddress(), 10n**14n, 175826565239047604n, 0, 3, 10n**12n
        )
    }).timeout(10000000000000000000000000000000);
