...
contract RouterUniswapEth is IRouterEvents, IRouterStructs, IRouterErrors{
    // @dev universal router interface
    IUniversalRouter constant ROUTER = IUniversalRouter(0x3fC91A3afd70395Cd496C647d5a6CC9D4B2b7FAD);
    // @dev WETH inteface
    IWNATIVE constant internal WETH_WRAPPER = IWNATIVE(0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2);
    // @dev uniswap v3 Factory
    IUniswapV3Factory constant internal V3_FACTORY = IUniswapV3Factory(0x1F98431c8aD98523631AE4a59f267346ea31F984);
    // @dev uniswap v2 Factory
    IUniswapV2Factory constant internal V2_FACTORY = IUniswapV2Factory(0x5C69bEe701ef814a2B6a3EDD4B1652CB9cc5aA6f);
    // @dev uniswap quoter interface
    IQuoterV2 constant internal QUOTER = IQuoterV2(0x61fFE014bA17989E743c5F6cB21bF9697530B21e);
    ...
    */
    function fetchTokenInfo(address token) external view returns (AllInfo memory info) {
        ...
        if (maxIndex == 0) {
            ...
            info.token0 = IUniswapV2Pair(pairAddresses[0]).token0();
            info.token1 = IUniswapV2Pair(pairAddresses[0]).token1();
            (uint112 token0R, uint112 token1R, ) = IUniswapV2Pair(pairAddresses[0]).getReserves();
    ...
    */
    function fetchTokensInfo(address[] calldata tokens) external view returns (GeneralInfo[] memory info) {
        ...
        for(uint i =0; i<len; i++){
            ...
            if (maxIndex == 0) {
                ...
                info[i].token0 = IUniswapV2Pair(pairAddresses[0]).token0();
                info[i].token1 = IUniswapV2Pair(pairAddresses[0]).token1();
                (uint112 token0R, uint112 token1R, ) = IUniswapV2Pair(pairAddresses[0]).getReserves();
    ...
    function fetchTokenPairs(address token) external view returns (address v2Pair, address v3_100Pair, address v3_500Pair, address v3_3000Pair, address v3_10000Pair) {
