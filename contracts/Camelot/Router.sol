...
import {IAlgebraFactory, ICamelotFactory, IAlegraPool, ICamelotPool, IQuoter, ISwapRouter, ICamelotRouter} from "./interfaces/IPoolFactories.sol";
import "hardhat/console.sol";

/// @author 0xCocomastoras
/// @custom:version 1.0
/// @title BlockHero Router
contract RouterCamelot is IRouterEvents, IRouterStructs, IRouterErrors{
    // @dev WETH inteface
    IWNATIVE constant internal WETH_WRAPPER = IWNATIVE(0x48b62137EdfA95a428D35C09E44256a739F6B557);
    // @dev uniswap v3 Factory
    IAlgebraFactory constant internal ALGEBRA_FACTORY = IAlgebraFactory(0x10aA510d94E094Bd643677bd2964c3EE085Daffc);
    // @dev uniswap v2 Factory
    ICamelotFactory constant internal CAMELOT_FACTORY = ICamelotFactory(0x7d8c6B58BA2d40FC6E34C25f9A488067Fe0D2dB4);

    ICamelotRouter constant internal CAMELOT_ROUTER = ICamelotRouter(0x18E621B64d7808c3C47bccbbD7485d23F257D26f);
    ISwapRouter constant internal SWAP_ROUTER = ISwapRouter(0xC69Dc28924930583024E067b2B3d773018F4EB52);
    IQuoter constant internal QUOTER = IQuoter(0x60A186019F81bFD04aFc16c9C01804a04E79e68B);
    ...
    */
    function sellNativeForToken(address tokenTo, uint256 minAmountOut, uint256 version, uint24 fee, bytes calldata) external payable {
        ...
        if(version == 0){
            ...
            path[1] = tokenTo;
            CAMELOT_ROUTER.swapExactETHForTokensSupportingFeeOnTransferTokens{value: amount}(minAmountOut, path, msg.sender, address(0), block.timestamp + 100);
            amountOut =  SafeTransferLib.balanceOf(tokenTo, msg.sender) - balancePre;
        }else if (version == 1){
            WETH_WRAPPER.deposit{value: amount}();
            SafeTransferLib.safeApproveWithRetry(address(WETH_WRAPPER), address(SWAP_ROUTER), amount);
            amountOut = SWAP_ROUTER.exactInputSingleSupportingFeeOnTransferTokens(
                ISwapRouter.ExactInputSingleParams(address(WETH_WRAPPER), tokenTo, msg.sender, block.timestamp + 100, amount, minAmountOut, 0)
            );
        }else {
            revert InvalidVersion();
        }
        emit SwapExecuted(msg.sender, address(WETH_WRAPPER), tokenTo, msg.value, amountOut, fee, version, pFees);
    }
    ...
    /**
        ...
        @param fee The pairs fee if it's a v3 or aeroCL
    */
    function sellTokenForNative(address tokenFrom, uint256 amountIn, uint256 minAmountOut, uint256 version, uint24 fee, bytes calldata) external {
        require(!userDenylist[msg.sender], 'DL');
        require(frozen == 0, 'CF');
        ...
        if(version == 0){
            SafeTransferLib.safeApproveWithRetry(address(tokenFrom), address(CAMELOT_ROUTER), amountIn);
            address[] memory path = new address[](2);
            path[1] = address(WETH_WRAPPER);
            path[0] = tokenFrom;
            CAMELOT_ROUTER.swapExactTokensForTokensSupportingFeeOnTransferTokens(amountIn, minAmountOut, path, address(this), address(0), block.timestamp + 100);
        }else if (version == 1){
            SafeTransferLib.safeApproveWithRetry(address(tokenFrom), address(SWAP_ROUTER), amountIn);
            SWAP_ROUTER.exactInputSingleSupportingFeeOnTransferTokens(
                ISwapRouter.ExactInputSingleParams(tokenFrom, address(WETH_WRAPPER), address(this), block.timestamp + 100, amountIn, minAmountOut, 0)
            );
        }else {
            revert InvalidVersion();
        }
    ...
    */
    function getOptimalPathBuy(address token, uint256 amountIn, uint256 slippage) external returns(uint256 version, uint256 maxAmountOut, uint256 fee, uint256 ){
        ...
        address camelotPair = CAMELOT_FACTORY.getPair(token, address(WETH_WRAPPER));
        address algebraPair = ALGEBRA_FACTORY.poolByPair(token, address(WETH_WRAPPER));
        if(camelotPair != address(0)) {
            address[] memory path = new address[](2);
            path[0] = address(WETH_WRAPPER);
            path[1] = token;
            uint256[] memory rsp = CAMELOT_ROUTER.getAmountsOut(amountInAfterFee, path);
            maxAmountOut = rsp[1];
            version = 0;
            fee = 0;
        }
        if (algebraPair != address(0)) {
            try QUOTER.quoteExactInputSingle(address(WETH_WRAPPER), token, amountInAfterFee, 0) returns (uint256 amountOut, uint16 feeOut){
    ...
    */
    function getOptimalPathSell(address token, uint256 amountIn, uint256 slippage) external returns(uint256 version, uint256 maxAmountOut, uint256 fee, uint256){
        ...
        if(camelotPair != address(0)) {
            ...
            path[1] = address(WETH_WRAPPER);
            path[0] = token;
            uint256[] memory rsp = CAMELOT_ROUTER.getAmountsOut(amountIn, path);
            maxAmountOut = rsp[1];
            version = 0;
            fee = 0;
        }
        if (algebraPair != address(0)) {
            try QUOTER.quoteExactInputSingle(token, address(WETH_WRAPPER), amountIn, 0) returns (uint256 amountOut, uint16 feeOut){
                if (amountOut > maxAmountOut) {
                    maxAmountOut = amountOut;
                    version = 1;
                    fee = uint256(feeOut);
    ...
    */
    ...
    function fetchTokenInfo(address token) external view returns (AllInfo memory info) {
        ...
        address[2] memory pairAddresses = [
            CAMELOT_FACTORY.getPair(token, address (WETH_WRAPPER)),
            ALGEBRA_FACTORY.poolByPair(token, address(WETH_WRAPPER))
        ];
         // @dev for efficiency we store all possible fees and 0 for v2
        uint24[2] memory fees = [uint24(0), uint24(60)];
        for (uint j=0; j<2; j++) {
        ...
        if (maxEthBalance != type(uint256).max) {
            if (maxIndex == 0) {
                ...
                info.token0 = ICamelotPool(pairAddresses[0]).token0();
                info.token1 = ICamelotPool(pairAddresses[0]).token1();
                (uint112 token0R, uint112 token1R, ,) = ICamelotPool(pairAddresses[0]).getReserves();
                info.reserves0 = token0R;
                info.reserves1 = token1R;
            } else {
                // @dev for v3 pairs
                info.pairAddress = pairAddresses[maxIndex];
                info.token0 = IAlegraPool(pairAddresses[maxIndex]).token0();
                info.token1 = IAlegraPool(pairAddresses[maxIndex]).token1();
                ...
                (uint160 sqrtPriceX96,,,,,,,) = IAlegraPool(pairAddresses[maxIndex]).globalState();
    ...
    */
    ...
    function fetchTokensInfo(address[] calldata tokens) external view returns (GeneralInfo[] memory info) {
        ...
        for(uint i =0; i<len; i++){
            ...
            address[2] memory pairAddresses = [
                CAMELOT_FACTORY.getPair(tokens[i], address (WETH_WRAPPER)),
                ALGEBRA_FACTORY.poolByPair(tokens[i], address(WETH_WRAPPER))
            ];
            // @dev for efficiency we store all possible fees and 0 for v2
            for (uint j=0; j<2; j++) {
                if (pairAddresses[j] != address(0)) {
            ...
            if (maxEthBalance != type(uint256).max) {
                ...
                if (maxIndex == 0) {
                    ...
                    info[i].token0 = ICamelotPool(pairAddresses[0]).token0();
                    info[i].token1 = ICamelotPool(pairAddresses[0]).token1();
                    (uint112 token0R, uint112 token1R, ,) = ICamelotPool(pairAddresses[0]).getReserves();
                    ...
                    info[i].token0 = IAlegraPool(pairAddresses[maxIndex]).token0();
                    info[i].token1 = IAlegraPool(pairAddresses[maxIndex]).token1();
    ...
    function fetchTokenPairs(address token) external view returns (address v2Pair, address v3Pair) {
        v2Pair = CAMELOT_FACTORY.getPair(token, address (WETH_WRAPPER));
        v3Pair = ALGEBRA_FACTORY.poolByPair(token, address(WETH_WRAPPER));
    }
    /**
    ...
    */
    function _findWethBalance(address pairAddress, uint24 fee) internal view returns(uint112 wethToken) {
        ...
        if(fee == 0) {
            ...
            address token0 = ICamelotPool(pairAddress).token0();
            (uint112 token0R, uint112 token1R,,) = ICamelotPool(pairAddress).getReserves();
